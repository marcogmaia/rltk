...
/*
 * Combines the functions/data required to render a virtual terminal, either as
 * root or as a layer. This is generally initialized by the library, not the
 * user.
 */
...
public:
    ...
    /*
     * Helper function that returns the index of a character location in the
     * backing vector.
     */
    inline int at(const int x, const int y) noexcept {
        return (y * term_width) + x;
    }

    /*
     * Set a character at backing-vector idx (use "at" to calculate) to the
     * specified target virtual character.
     */
     ...
     */
    inline void set_char(const int x, const int y, vchar target) noexcept {
        set_char(at(x, y), target);
    }

    /*
     * Print a string to the terminal, at location x/y, string s, with
     * foreground and background of fg and bg. If you don't specify colors, it
     * will do white on black.
     */
    void print(const int x, const int y, const std::string& s,
               const color_t& fg = colors::WHITE,
               const color_t& bg = colors::BLACK) noexcept;

    /*
     * Center a string (relative to the terminal size), and print it at location
     * y.
     */
    void print_center(const int y, const std::string& s,
                      const color_t& fg = colors::WHITE,
                      const color_t& bg = colors::BLACK) noexcept;

    /*
     * Draw a box taht encompasses the whole terminal boundary, in color fg/bg.
     * If double_lines is set to true, then it will use the two-line/thick box
     * characters.
     */
    inline void box(const color_t& fg = colors::WHITE,
                    const color_t& bg = colors::BLACK,
                    bool double_lines = false) noexcept {
        box(0, 0, term_width - 1, term_height - 1, fg, bg, double_lines);
    }

    /*
     * Draw a box at x/y of size w/h, in color fg/bg (or black/white if not
     * specified). If double_lines is true, it will use the two-line/thick box
     * characters.
     */
     ...
     */
    void fill(const int left_x, const int top_y, const int right_x,
              const int bottom_y, const uint8_t glyph,
              const color_t& fg = colors::WHITE,
              const color_t& bg = colors::BLACK) noexcept;
    ...
    /*
     * Renders the terminal to the specified renderable. Don't call this
     * directly - the toolkit will take care of it.
     */
    void render(sf::RenderTarget& window);

    /*
     * Sets the global translucency level for the console. You can use this to
     * make a translucent console layer on top of other items.
     */
    inline void set_alpha(const uint8_t new_alpha) noexcept {
        alpha = new_alpha;
    }
     ...
     */
    inline void set_tint(const color_t new_tint) noexcept {
        tint = new_tint;
    }
    ...
    /*
     * This gets the current font size in pixels, first is width, second is
     * height.
     */
