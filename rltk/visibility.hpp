...
void internal_2d_sweep(const location_t_& position, const int& range,
    ...
    line_func_cancellable(start_x, start_y, end_x, end_y,
                          [&blocked, &is_transparent, &set_visible, &range,
                           &start_x, &start_y](int X, int Y) {
                              if(blocked)
                                  return false;
                              float distance
                                  = distance2d(start_x, start_y, X, Y);
                              if(distance <= range) {
...
/* Simple all-direction visibility sweep in 2 dimensions. This requires that
 * your location_t utilize an x and y component. Parameters: position - where
 * you are sweeping from. range - the number of tiles you can traverse.
 * set_visible - a callback (such as bool set_visible(location_t & loc)) to say
 * "this is visible" is_transparent - a callback to ask your map if you can see
 * through a tile.
 *
 * You must provide a navigator_t, just like for path finding. It must support
